...
public class PacketsManager
{
    ...
    public static PacketContainer[][] constructSpawnPackets(final Player observer, Disguise disguise, Entity disguisedEntity)
    {
        ...
        if (DisguiseConfig.isMiscDisguisesForLivingEnabled())
        {
            if (disguise.getWatcher() instanceof LivingWatcher)
            {
                ...
                {
                    if (DisguiseConfig.isMaxHealthDeterminedByDisguisedEntity() && disguisedEntity instanceof Damageable)
                    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                {
                    if (disguise.getType().isMob() || disguise.getType() == DisguiseType.ARMOR_STAND)
                    {
    ...
    /**
     * Transform the packet magically into the one I have always dreamed off. My
     * true luv!!! This will return null if its not transformed
     */
    public static PacketContainer[][] transformPacket(PacketContainer sentPacket, Player observer, Entity entity)
    {
        ...
        try
        {
            ...
            if (disguise != null)
            {
                    ...
                    } else
                    {
                        ...
                        for (WrappedAttribute attribute : sentPacket.getAttributeCollectionModifier().read(0))
                        {
                            if (attribute.getAttributeKey().equals("generic.maxHealth"))
                            {
                                ...
                                {
                                    if (DisguiseConfig.isMaxHealthDeterminedByDisguisedEntity())
                                    {
                ...
                {
                    if (sentPacket.getType() == Server.ENTITY_METADATA)
                    {
                    ...
                    {
                        if (sentPacket.getType() == Server.NAMED_ENTITY_SPAWN || sentPacket.getType() == Server.SPAWN_ENTITY_LIVING
                        ...
                        {
                            if (sentPacket.getType() == Server.ANIMATION)
                            {
                            ...
                            {
                                if (sentPacket.getType() == Server.COLLECT)
                                {
                                    if (disguise.getType().isMisc())
                                    {
                                        packets = new PacketContainer[0];
                                    ...
                                    {
                                        if (DisguiseConfig.isBedPacketsEnabled() && disguise.getType().isPlayer()
                                ...
                                {
                                    if (sentPacket.getType() == Server.REL_ENTITY_MOVE_LOOK || sentPacket.getType() == Server.ENTITY_LOOK
                                            || sentPacket.getType() == Server.ENTITY_TELEPORT || sentPacket.getType() == Server.REL_ENTITY_MOVE)
                                    {
                                        ...
                                        {
                                            if (sentPacket.getType() != Server.REL_ENTITY_MOVE)
                                            {
                                    ...
                                    {
                                        if (sentPacket.getType() == Server.ENTITY_EQUIPMENT)
                                        {
                                        ...
                                        {
                                            if (sentPacket.getType() == Server.BED)
                                            {
                                                if (!disguise.getType().isPlayer())
                                                {
                                                    packets = new PacketContainer[0];
                                            ...
                                            {
                                                if (sentPacket.getType() == Server.ENTITY_STATUS)
                                                {
                                                ...
                                                {
                                                    if (sentPacket.getType() == Server.ENTITY_HEAD_ROTATION)
                                                    {
